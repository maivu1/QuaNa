...
function animate() {
  ...
  let hintText; // để dùng ngoài hàm animate
...
function createHintText() {
    ...
    // ===== LỚP 1: VIỀN VÀNG NỔI BẬT =====
    context.shadowColor = 'transparent';
    context.lineWidth = 10;
    context.strokeStyle = '#FFD700';
    context.strokeText(text, canvasSize / 2, canvasSize / 2);

    // ===== LỚP 2: VIỀN GLOW XANH DƯƠNG =====
    context.shadowColor = '#00bfff';
    context.shadowBlur = 35;
    context.lineWidth = 4;
    context.strokeStyle = '#aafaff';
    context.strokeText(text, canvasSize / 2, canvasSize / 2);

    // ===== LỚP 3: CHỮ TRẮNG CHÍNH =====
    context.shadowColor = '#00bfff';
    context.shadowBlur = 15;
    context.fillStyle = '#ffffff';
    context.fillText(text, canvasSize / 2, canvasSize / 2);

    // ===== LỚP 4: VIỀN XANH NGỌC NHẸ =====
    context.shadowColor = 'transparent';
    context.lineWidth = 2;
    context.strokeStyle = '#7fffd4';
    context.strokeText(text, canvasSize / 2, canvasSize / 2);

    // ===== TẠO TEXTURE CHO THREE.JS =====
    ...
    // Bắt đầu với alpha thấp để có hiệu ứng mờ
    hintText.material.opacity = 0.6;

    scene.add(hintText);
}

// ===== HIỆU ỨNG CHỚP SÁNG NHẸ (PULSING) =====
let glowSpeed = 2.0; // tốc độ chớp
function animateHintText(deltaTime) {
    if (!hintText) return;

    // Công thức sin để dao động alpha giữa 0.6 ↔ 1.0
    const time = performance.now() * 0.002 * glowSpeed;
    hintText.material.opacity = 0.6 + Math.abs(Math.sin(time)) * 0.4;
}
